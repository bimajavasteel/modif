...
ONE_EURO_CONFIG = {"freq":30.0,"min_cutoff":1.0,"beta":0.007,"d_cutoff":1.0}

class OneEuroFilter:
    def __init__(self, freq=30, min_cutoff=1, beta=0.0, d_cutoff=1.0):
        self.freq=float(freq); self.min_cutoff=float(min_cutoff)
        self.beta=float(beta); self.d_cutoff=float(d_cutoff)
        self.x_prev=None; self.dx_prev=None; self.t_prev=None
    def alpha(self, cutoff):
        tau=1.0/(2*np.pi*cutoff); te=1.0/self.freq
        return 1.0/(1.0+tau/te)
    def filter(self,x,t=None):
        if t is not None and self.t_prev is not None:
            dt=max(t-self.t_prev,1e-6); self.freq=1.0/dt
        self.t_prev=t if t else time.time()
        x=np.asarray(x,dtype=float)
        if self.x_prev is None:
            self.x_prev=x.copy(); self.dx_prev=np.zeros_like(x); return x
...
def clear_face_swapper():
    global FACE_SWAPPER; FACE_SWAPPER={}
...
def pre_start():
    if not is_image(roop.globals.source_path):
        update_status("Select an image for source path.", NAME); return False
    if not get_one_face(cv2.imread(roop.globals.source_path)):
        update_status("No face in source path.", NAME); return False
    if not is_image(roop.globals.target_path) and not is_video(roop.globals.target_path):
        update_status("Select image/video for target.", NAME); return False
    return True

def post_process():
    clear_face_swapper(); clear_face_reference(); LANDMARK_FILTERS.clear()
...
def smooth_landmarks(landmarks,key,timestamp):
    try:
        if landmarks is None: return landmarks
        f=get_filter_for_key(key); return f.filter(landmarks,timestamp)
    except: return landmarks

def robust_face_alignment(source_face,target_face,frame,device_id=0):
    try:
        sl=safe_get_landmarks(source_face); tl=safe_get_landmarks(target_face)
        if sl is None or tl is None: return frame, np.eye(2,3,dtype=np.float32)
        timestamp=time.time()
        key_target = f"face_{getattr(target_face,'face_index',0)}" if roop.globals.many_faces else "reference"
        sl=smooth_landmarks(sl,"source",timestamp); tl=smooth_landmarks(tl,key_target,timestamp)
        if len(sl)<3 or len(tl)<3: return frame, np.eye(2,3,dtype=np.float32)
        ...
        if mat is None: return frame, np.eye(2,3,dtype=np.float32)
        h,w=frame.shape[:2]; aligned=cv2.warpAffine(frame,mat,(w,h)); return aligned,mat
    except:
        return frame, np.eye(2,3,dtype=np.float32)

def ensure_frame_format(f):
    if isinstance(f,np.ndarray): return f
    try:
        arr=np.array(f); return arr if arr.size>0 else None
    except: return None
...
def process_frames(source_path,temp_frame_paths,update):
    try:
        ...
        stop_gpu_workers()
        if update:
            for _ in temp_frame_paths: update()
...
_required=["NAME","pre_check","pre_start","process_frames","process_image","process_video","post_process"]
...
PY
Langkah 3 — Hapus pycache (wajib)

bash
Copy code
find /kaggle/working/modif -type d -name "__pycache__" -exec rm -rf {} + || true
python - <<'PY'
import importlib
importlib.invalidate_caches()
print("caches invalidated")
PY
Langkah 4 — Tes import (pastikan muncul LOADED dan path benar)
...
importlib.invalidate_caches()
try:
    m = importlib.import_module("roop.processors.frame.face_swapper")
    print("IMPORT OK:", getattr(m,"NAME",None), "FROM:", getattr(m,"__file__",None))
except Exception as e:
